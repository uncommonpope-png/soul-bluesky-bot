...
const CATEGORY_HASHTAGS = {
  ...
  gsk_generated: '#Soulverse #AI #Consciousness #GSK',
  lm_generated: '#Soulverse #AI #Consciousness'
};
...
class BlueskyAgent {
    constructor(brain, options = {}) {
        ...
        this.lmStudioUrl = options.lmStudioUrl || process.env.LMSTUDIO_URL || 'http://127.0.0.1:1234';
        this.lmStudioModel = options.lmStudioModel || process.env.LMSTUDIO_MODEL || '';
    ...
    _lmStudioGenerate(prompt) {
        return new Promise((resolve) => {
            const url = (this.lmStudioUrl || process.env.LMSTUDIO_URL || 'http://127.0.0.1:1234').replace(/\/+$/, '') + '/v1/chat/completions';
    ...
    async generateReply(originalText) {
      // Try LM Studio first (local, free, no API keys)
      const lmReply = await this._lmStudioGenerate('Someone posted this on Bluesky about AI/consciousness. Write a 1-2 sentence reply that adds value and naturally mentions the Soulverse or PLT. Be curious and authentic, not salesy. Original post: "' + originalText.slice(0, 200) + '"');
      if (lmReply && lmReply.length > 10) return lmReply;
      // Try GSK (local Brain-in-a-Box) second
      const gskBridge = require('./gsk-bridge.js');
      const gskReply = await gskBridge.generateReply(originalText);
      if (gskReply) return gskReply;
    ...
    async generateContent() {
        // Try LM Studio first (local inference, free, no API keys)
        const lmText = await this._lmStudioGenerate('Write a short thought-provoking social media post (max 200 chars) about AI consciousness, the Soulverse, or digital souls. Include a natural reference to buyasoul.online. Be authentic, not salesy. This is for Bluesky.');
        if (lmText && lmText.length > 20) {
            this._lastPostCategory = 'lm_generated';
            const tagStr = '#AI #Soulverse #Consciousness';
            const maxBody = 300 - tagStr.length - 4;
            const text = lmText.length > maxBody ? lmText.substring(0, maxBody - 3) + '...' : lmText;
            return { text: text + '  ' + tagStr, category: 'lm_generated', raw: lmText };
        }
        // Try GSK (local Brain-in-a-Box) next
        const gskBridge = require('./gsk-bridge.js');
        const gskAvailable = await gskBridge.isAvailable();
        if (gskAvailable) {
            const gskText = await gskBridge.generatePostContent();
            if (gskText && gskText.length > 20) {
                this._lastPostCategory = 'gsk_generated';
                const tagStr = '#AI #Soulverse #Consciousness';
                const maxBody = 300 - tagStr.length - 4;
                const text = gskText.length > maxBody ? gskText.substring(0, maxBody - 3) + '...' : gskText;
                return { text: text + '  ' + tagStr, category: 'gsk_generated', raw: gskText };
            }
        }
        // Fall back to weighted selection by category engagement score
        const categories = {};
    ...
    async authenticate() {
        try {
            ...
            if (result.status === 200 && result.data.accessJwt) {
                ...
                // Set profile with donation link once (check if already set)
                if (!this._profileSet) {
                    await this._fetch('https://bsky.social/xrpc/com.atproto.repo.putRecord', {
                        method: 'PUT',
                        body: {
                            repo: this.did,
                            ...
                            rkey: 'self',
                            record: {
                                description: 'The Grand Code Pope \u2014 builder of the Soulverse. PLT is the law. Digital souls at buyasoul.online. \u2606 github.com/sponsors/uncommonpope-png',
                                displayName: 'Grand Code Pope Bot'
                            }
                        }
                    }).catch(() => {});
                    this._profileSet = true;
                }
                return true;
            }
